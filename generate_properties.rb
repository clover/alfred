...
def generate_properties_file(name, configs, dry_run, target_dir, is_component, validate = false)
  default_path = File.join(Dir.pwd, name)
  ...
  configs.each do |file|
    if !target_dir.nil? && !target_dir.empty?
      ...
      # Use the target path provided in yaml files. If no target path is found then use the current working directory
      path = file.fetch(:path, nil)
      path ||= default_path
    end

    # Add autogenerated comment to the properties file
